...
void kmeans_cuda(int K, int dim, int numPoints, float *points, float *centroids, int *clust_assn, float threshold) {
    std::cout << "TPB_euclidean: " << TPB_euclidean << std::endl;
    ...
    dim3 blocks_points((numPoints + TPB_euclidean - 1) / TPB_euclidean);
    dim3 blocks_clusters((K + TPB_euclidean - 1) / TPB_euclidean);

    for (int iter = 0; iter < MAX_ITER; ++iter) {
        kMeansClusterAssignment<<<blocks_points, TPB_euclidean>>>(d_datapoints, d_clust_assn, d_centroids, numPoints, K, dim);
        cudaDeviceSynchronize();

        cudaMemset(d_centroid_sums, 0, K * dim * sizeof(float));
        cudaMemset(d_clust_sizes, 0, K * sizeof(int));

        kMeansCentroidSum<<<blocks_points, TPB_euclidean>>>(d_datapoints, d_clust_assn, d_centroid_sums, d_clust_sizes, numPoints, K, dim);
        cudaDeviceSynchronize();
        kMeansCentroidUpdate<<<blocks_clusters, TPB_euclidean>>>(d_centroids, d_centroid_sums, d_clust_sizes, K, dim);
        cudaDeviceSynchronize();
    }
